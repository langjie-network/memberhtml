(function (window) {
    ...
    M.getljToken=function (){
        return {
            lj_token: M.userInfo.lj_token
        }
    }
        ...
        if(res.code==-100 || res.code==-6){
            if(M.isWeiXin){
              ...
              //  location.href=`/member/liuyanban?redirect_uri=${redirect_uri}`;
                ...
                location.href=`/memberhtml/liuyanban/index.html#/login`;
            }
        }
    }
    ...
    function preHeader(headers){
        ...
        let tokenObj= M.getljToken();
        if(tokenObj!=null){
            reqHeader.token=tokenObj.lj_token;
        }
        return reqHeader;
    }
