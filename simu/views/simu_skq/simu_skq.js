...
M.loadCss("/memberhtml/simu/views/simu_skq/simu_skq.css")
...
<div v-cloak id="shoukongqi"
    ...
    <div class="online_div"  >
        <div class="online_div_zaixian">
            <img class="left_icon" src="https://langjie.oss-cn-hangzhou.aliyuncs.com/space/root/project/shoukongqi/static/yonghu_img.png" />
            <div class="mid">在线{{clientList.length}}人</div> 
            <div></div> 
        </div> 
        <div class="online_div_opera">
            <img class="left_icon" src="https://langjie.oss-cn-hangzhou.aliyuncs.com/space/root/project/shoukongqi/static/shiyan_img.png" />
            <div class="mid">操作者: {{$common.secUserName(occupyClient.createUser)}}</div> 
            <img @click="showClientList" class="right_icon" :src="$BASE_IMG_URL(zhankaiImg)" />
        </div>
         <div v-show="zhankaiImg=='zhankai_up.png'" class="clientListWrap">
                <div class="online_div_clientListWrap_head">
                      <template v-if="myClientIndex==0">
                            正在操作中
                      </template>
                      <template v-else>
                            当前排在第{{myClientIndex}}位
                       </template>
                </div>
                <ul class="online_div_clientListWrap_ul">
                    <li  v-for="(item,i) in paiMingClientList">
                         <div class="clientListIndex">
                               {{i+1}}.
                            </div>
                            <div class="clientListName">
                               {{$common.secUserName(item.createUser)}}
                            </div>
                            <div class="clientListTail">
                               &nbsp;
                            </div>
                    </li>
                </ul>
...
export default {
                    ...
                    if(client.terminalType=="wx"){
                       ...
                       // debugger
                        ...
                        if(this.$socketIoWebPlugin.clientId==client.clientId){
                            this.myClientIndex=weXinClientCount-1;
                        }
                    }
    ...
    async mounted() {
        ...
        $(".activeImg").hide()
        $(".defaultImg").show()
        M.Component.simu_skq=this;
        if(M.userInfo!=null){
            await MIO.socketConnect();
        }
    ...
    computed: {
        ...
        paiMingClientList(){
            return this.clientList.filter((v,i)=>i>0);
        }
    }
